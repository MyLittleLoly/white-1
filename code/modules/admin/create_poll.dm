/client/proc/create_poll()
	...
	var/DBQuery/query_validate_time = dbcon.NewQuery("SELECT IF(STR_TO_DATE('[endtime]','%Y-%c-%d %T') > NOW(), STR_TO_DATE('[endtime]','%Y-%c-%d %T'), 0)")
	...
	if(polltype != POLLTYPE_TEXT)
		var/pollid = 0
		var/DBQuery/query_get_id = dbcon.NewQuery("SELECT LAST_INSERT_ID()")
