...
/datum/admins/proc/DB_ban_record(bantype, mob/banned_mob, duration = -1, reason, job = "", banckey = null, banip = null, bancid = null)
	...
	var/datum/DBQuery/query_add_ban_get_ckey = dbcon.NewQuery("SELECT ckey FROM [format_table_name("player")] WHERE ckey = '[ckey]'")
	...
	if(maxadminbancheck)
		var/datum/DBQuery/query_check_adminban_amt = dbcon.NewQuery("SELECT count(id) AS num FROM [format_table_name("ban")] WHERE (a_ckey = '[a_ckey]') AND (bantype = 'ADMIN_PERMABAN'  OR (bantype = 'ADMIN_TEMPBAN' AND expiration_time > Now())) AND isnull(unbanned)")
	...
	var/datum/DBQuery/query_add_ban = dbcon.NewQuery(sql)
...
/datum/admins/proc/DB_ban_unban(ckey, bantype, job = "")
	...
	var/datum/DBQuery/query_unban_get_id = dbcon.NewQuery(sql)
...
/datum/admins/proc/DB_ban_edit(banid = null, param = null)
	...
	var/datum/DBQuery/query_edit_ban_get_details = dbcon.NewQuery("SELECT ckey, duration, reason FROM [format_table_name("ban")] WHERE id = [banid]")
	...
	switch(param)
		if("reason")
			...
			var/datum/DBQuery/query_edit_ban_reason = dbcon.NewQuery("UPDATE [format_table_name("ban")] SET reason = '[value]', edits = CONCAT(edits,'- [eckey] changed ban reason from <cite><b>\\\"[reason]\\\"</b></cite> to <cite><b>\\\"[value]\\\"</b></cite><BR>') WHERE id = [banid]")
		...
		if("duration")
			...
			var/datum/DBQuery/query_edit_ban_duration = dbcon.NewQuery("UPDATE [format_table_name("ban")] SET duration = [value], edits = CONCAT(edits,'- [eckey] changed ban duration from [duration] to [value]<br>'), expiration_time = DATE_ADD(bantime, INTERVAL [value] MINUTE) WHERE id = [banid]")
...
/datum/admins/proc/DB_ban_unban_by_id(id)
	...
	var/pckey
	var/datum/DBQuery/query_unban_get_ckey = dbcon.NewQuery(sql)
	...
	var/datum/DBQuery/query_unban = dbcon.NewQuery(sql_update)
...
/datum/admins/proc/DB_ban_panel(playerckey = null, adminckey = null, page = 0)
	...
	if(adminckey || playerckey)
		...
		var/pagecount = 0
		page = text2num(page)
		var/datum/DBQuery/query_count_bans = dbcon.NewQuery("SELECT COUNT(id) FROM [format_table_name("ban")] WHERE 1 [playersearch] [adminsearch]")
		...
		var/datum/DBQuery/query_search_bans = dbcon.NewQuery("SELECT id, bantime, bantype, reason, job, duration, expiration_time, ckey, a_ckey, unbanned, unbanned_ckey, unbanned_datetime, edits FROM [format_table_name("ban")] WHERE 1 [playersearch] [adminsearch] ORDER BY bantime DESC[limit]")
