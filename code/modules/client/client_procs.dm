...
/client/proc/set_client_age_from_db(connectiontopic)
	...
	var/datum/DBQuery/query_get_related_ip = dbcon.NewQuery("SELECT ckey FROM [format_table_name("player")] WHERE ip = INET_ATON('[address]') AND ckey != '[sql_ckey]'")
	query_get_related_ip.Execute()
	related_accounts_ip = ""
	while(query_get_related_ip.NextRow())
		related_accounts_ip += "[query_get_related_ip.item[1]], "
	var/datum/DBQuery/query_get_related_cid = dbcon.NewQuery("SELECT ckey FROM [format_table_name("player")] WHERE computerid = '[computer_id]' AND ckey != '[sql_ckey]'")
	...
	var/new_player
	var/datum/DBQuery/query_client_in_db = dbcon.NewQuery("SELECT 1 FROM [format_table_name("player")] WHERE ckey = '[sql_ckey]'")
	...
	if(!query_client_in_db.NextRow())
		...
		var/datum/DBQuery/query_add_player = dbcon.NewQuery("INSERT INTO [format_table_name("player")] (`ckey`, `firstseen`, `lastseen`, `ip`, `computerid`, `lastadminrank`, `accountjoindate`) VALUES ('[sql_ckey]', Now(), Now(), INET_ATON('[sql_ip]'), '[sql_computerid]', '[sql_admin_rank]', [account_join_date ? "'[account_join_date]'" : "NULL"])")
		if(!query_add_player.Execute())
			return
		if(!account_join_date)
			account_join_date = "Error"
			account_age = -1
	var/datum/DBQuery/query_get_client_age = dbcon.NewQuery("SELECT firstseen, DATEDIFF(Now(),firstseen), accountjoindate, DATEDIFF(Now(),accountjoindate) FROM [format_table_name("player")] WHERE ckey = '[sql_ckey]'")
		...
		if(!account_join_date)
			...
			if(!account_age)
				...
				if(!account_join_date)
					...
					var/datum/DBQuery/query_datediff = dbcon.NewQuery("SELECT DATEDIFF(Now(),[account_join_date])")
					if(!query_datediff.Execute())
						return
					if(query_datediff.NextRow())
						account_age = text2num(query_datediff.item[1])
	if(!new_player)
		var/datum/DBQuery/query_log_player = dbcon.NewQuery("UPDATE [format_table_name("player")] SET lastseen = Now(), ip = INET_ATON('[sql_ip]'), computerid = '[sql_computerid]', lastadminrank = '[sql_admin_rank]', accountjoindate = [account_join_date ? "'[account_join_date]'" : "NULL"] WHERE ckey = '[sql_ckey]'")
		if(!query_log_player.Execute())
			return
	if(!account_join_date)
		account_join_date = "Error"
	var/datum/DBQuery/query_log_connection = dbcon.NewQuery("INSERT INTO `[format_table_name("connection_log")]` (`id`,`datetime`,`server_ip`,`server_port`,`ckey`,`ip`,`computerid`) VALUES(null,Now(),INET_ATON(IF('[world.internet_address]' LIKE '', '0', '[world.internet_address]')),'[world.port]','[sql_ckey]',INET_ATON('[sql_ip]'),'[sql_computerid]')")
...
/client/proc/check_randomizer(topic)
	...
	if (oldcid)
		...
		var/datum/DBQuery/query_cidcheck = dbcon.NewQuery("SELECT computerid FROM [format_table_name("player")] WHERE ckey = '[sql_ckey]'")
...
/client/proc/note_randomizer_user()
	...
	var/datum/DBQuery/query_get_notes = dbcon.NewQuery("SELECT id FROM [format_table_name("messages")] WHERE type = 'note' AND targetckey = '[sql_ckey]' AND adminckey = '[adminckey]' AND timestamp + INTERVAL 1 DAY < NOW()")
