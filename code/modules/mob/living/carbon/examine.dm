/mob/living/carbon/examine(mob/user)
	...
	if (head)
		msg += "[t_He] [t_is] wearing [icon(head)] \a [src.head] on [t_his] head. \n"
	if (wear_mask)
		msg += "[t_He] [t_is] wearing [icon(wear_mask)] \a [src.wear_mask] on [t_his] face.\n"
	if (wear_neck)
		msg += "[t_He] [t_is] wearing [icon(wear_neck)] \a [src.wear_neck] around [t_his] neck.\n"
	...
	if (back)
		msg += "[t_He] [t_has] [icon(back)] \a [src.back] on [t_his] back.\n"
	var/appears_dead = 0
	if (stat == DEAD)
		appears_dead = 1
